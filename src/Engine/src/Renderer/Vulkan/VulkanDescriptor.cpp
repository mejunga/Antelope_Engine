...
namespace Antelope {
    ...
    VkDescriptorPool DescriptorAllocator::CreatePool(uint32_t setCount, std::vector<PoolSizeRatio> poolRatios)
    {
        ...
        for (auto r : poolRatios) {
            poolSizes.push_back({r.type, uint32_t(r.ratio * setCount)});
        }
    ...
    {
        VkDescriptorBufferInfo& info = m_BufferInfos.emplace_back(VkDescriptorBufferInfo{buffer, offset, size});
    ...
    {
        VkDescriptorImageInfo& info = m_ImageInfos.emplace_back(VkDescriptorImageInfo{sampler, imageView, layout});
        VkWriteDescriptorSet write{};
        write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        write.dstBinding = binding;
